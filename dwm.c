...
void drawbar(Monitor *m) {
    ...
    if ((w = m->ww - tw - x) > bh) {
        if (n > 0) {
            int remainder = w % n;
            int tabw_max = (1.0 / (double)n) * ((double)w * 0.9) + 1;
            int tabw = tabw_max < 100 ? tabw_max : 100;
            for (c = m->clients; c; c = c->next) {
                if (!ISVISIBLE(c))
                    continue;
                if (m->sel == c)
...
void setup(void) {
    ...
    for (i = 0; i < LENGTH(barcolors) && i < LENGTH(barclrs); i++)
        drw_clr_create(drw, &barclrs[i], barcolors[i], OPAQUE);
    if (i == 0)
        drw_clr_create(drw, &barclrs[i++], "#000000", OPAQUE);
    for (; i < 7; i++) {
        snprintf(cbuf, sizeof(cbuf), "#%02x%02x%02x", !!(i & 1) * 0x7f,
                 !!(i & 2) * 0x7f, !!(i & 4) * 0x7f);
        drw_clr_create(drw, &barclrs[i], cbuf, OPAQUE);
    }
    if (i == 7)
        drw_clr_create(drw, &barclrs[i++], "#000000", OPAQUE);
    if (i == 8)
        drw_clr_create(drw, &barclrs[i++], "#333333", OPAQUE);
    for (; i < 16; i++) {
...
void fibonacci(Monitor *mon, int s) {
    unsigned int i, n, nx, ny, nw, nh, gappx;
    Client *c;
    ...
    for (i = 0, c = nexttiled(mon->clients); c; c = nexttiled(c->next)) {
        if ((i % 2 && nh / 2 > 2 * (c->bw + gappx)) ||
            (!(i % 2) && nw / 2 > 2 * (c->bw + gappx))) {
            if (i < n - 1) {
                if (i % 2)
                    nh /= 2;
                else
        ...
        resize(c, nx + gappx, ny + gappx, nw - 2 * c->bw - 2 * gappx,
               nh - 2 * c->bw - 2 * gappx, False);
    }
}
